...
const Modal = (props:ModalProps) => {
  ...
  const baseUrl = "http://localhost:5000/api/admin/signup";
  ...
  return (
    <div className={`${styles.modal} ${props.show ? styles.show : ''}`}>
      <div className={styles.modalContent}>
        <span className={styles.modalclose} onClick={props.handleClose}>&times;</span>
        <h3>Enter Admin Details</h3> <br />
        <form className={styles.modalform} onSubmit={onSubmit}>
          <label htmlFor="adminId" className={styles.modallabel}>Admin ID:</label>
          <input type="text" id="adminId" name="adminId" required className={styles.modalinput} 
          value={AddAdmin.adminId}
          onChange={handle}
          />

          <label htmlFor="password"  className={styles.modallabel}>Password:</label>
          <input type="password" id="password" name="password" required  className={styles.modalinput}
          value={AddAdmin.password}
          onChange={handle}
          />

          <button type="submit" className={styles.modalbutton} onClick={handleclose}>Submit</button>
        </form>
      </div>
    </div>
  );
